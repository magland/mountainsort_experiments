function firings=kilosort_wrapper(RAW,options)

if (nargin<1) kilosort_wrapper_test; return; end;

if (nargin<2) options=struct; end;
if (~isfield(options,'samplerate')) options.samplerate=30000; end;

addpath /home/magland/dev/KiloSort
addpath /home/magland/dev/KiloSort/mergesplits
addpath /home/magland/dev/npy-matlab

tmp_raw_fname=[make_temporary_fname,'.dat'];

samplerate=options.samplerate;
M=size(RAW,1); %number of channels
N=size(RAW,2); %number of timepoints

write_raw(RAW,tmp_raw_fname);

ops=get_default_ops(samplerate,M);
ops.fbinary=tmp_raw_fname;

ops.fproc=[make_temporary_fname,'.dat'];
disp('************************* preprocessing...');
...
function kilosort_wrapper_test
mfile_path=fileparts(mfilename('fullpath'));
%X=readmda('/home/magland/dev/mountainsort_experiments/BIGFILES/synth_examples/example1/raw.mda');
X=readmda('/home/magland/dev/mountainsort_experiments/BIGFILES/neuron_paper/tetrode/20160426_kanye_02_r1.nt16.mda');
firings=kilosort_wrapper(X);
writemda(firings,[mfile_path,'/firings_ks_tet.mda']);
end

function write_raw(X,fname)
F=fopen(fname,'wb');
fwrite(F,X(:),'int16');
fclose(F);
end
...
function fname=make_temporary_fname
...
function ops=get_default_ops(samplerate,num_channels)
...
ops.fs                  = samplerate;        % sampling rate		
ops.NchanTOT            = num_channels;           % total number of channels		
ops.Nchan               = num_channels;           % number of active channels 		
...
%ops.chanMap             = '/home/magland/dev/KiloSort/test1/chanMap.mat'; % make this file using createChannelMapFile.m		
