function [labels,output]=isosplit4(X,opts)
global global_timers;
global_timers=zeros(1,10);
...
if (~isfield(opts,'K_init')) opts.K_init=500; end;
...
timers.test_redistribute=0;
timers.merging=0;
timers.redistributing=0;
timers.find_next_comparisons=0;
timers.kmeans=0;

t1=tic;
%labels_init=local_kmeans_sorber(X,opts.K_init);
labels_init=initialize_clustering(X,opts.K_init);

%counts=accumarray(labels_init',1,[max(labels_init),1])';
%figure; hist(counts,100);
%return;

timers.kmeans=timers.kmeans+toc(t1);
labels=labels_init;
...
dists_between_centers=get_dists_between_centers(centers);
...
while 1
    ...
    [k1s,k2s]=find_next_comparisons(active_labels,centers,dists_between_centers,weighted_counts,attempted_comparisons,opts.repeat_tolerance);
    timers.find_next_comparisons=timers.find_next_comparisons+toc(t1);
    if (length(k1s)==0) break; end;
    fprintf('Found %d comparisons.\n',length(k1s));
    for iiii=1:length(k1s)
        k1=k1s(iiii);
        k2=k2s(iiii);
        if (opts.return_iterations)
        ...
        timers.test_redistribute=timers.test_redistribute+toc(t1);
        if (opts.return_iterations)
            iteration_info.projection=info0.projection;
            iteration_info.projection_weights=info0.projection_weights;
            iteration_info.inds_for_comparision=info0.inds_for_comparison;
        ...
        if (do_merge)||(max(labels0)==1)
            ...
            dists_between_centers(active_inds,k1)=get_dists_between_centers_and_point(centers(:,active_inds),centers(:,k1));
            dists_between_centers(k1,active_inds)=dists_between_centers(active_inds,k1);
            weighted_counts(k1)=sum(weights(inds12));
            weighted_counts(k2)=0;
            active_labels(k2)=0;
            timers.merging=timers.merging+toc(t1);
        else
            t1=tic;
            ...
            dists_between_centers(active_inds,k1)=get_dists_between_centers_and_point(centers(:,active_inds),centers(:,k1));
            dists_between_centers(k1,active_inds)=dists_between_centers(active_inds,k1);
            dists_between_centers(active_inds,k2)=get_dists_between_centers_and_point(centers(:,active_inds),centers(:,k2));
            dists_between_centers(k2,active_inds)=dists_between_centers(active_inds,k2);
            weighted_counts(k1)=sum(weights(indsA));
            weighted_counts(k2)=sum(weights(indsB));
            timers.redistributing=timers.redistributing+toc(t1);
        end;
        if (opts.return_iterations)
...
function dists=get_dists_between_centers(centers)
[M,N]=size(centers);
dists=zeros(N,N);
for m=1:M
...
function dists=get_dists_between_centers_and_point(centers,pt);
[M,N]=size(centers);
diffs=centers-repmat(pt,1,N);
dists=sqrt(sum(diffs.^2,1));

function [k1s,k2s]=find_next_comparisons(active_labels,centers,dists_between_centers,weighted_counts,attempted_comparisons,repeat_tolerance)
global global_timers
k1s=[];
k2s=[];
t1=tic;
returning_these_labels=zeros(size(active_labels));
active_inds=find(active_labels);
centers_active=centers(:,active_inds);
weighted_counts_active=weighted_counts(active_inds);
Na=size(centers_active,2);
dists=dists_between_centers(active_inds,active_inds);
global_timers(1)=global_timers(1)+toc(t1); t1=tic;
for j=1:Na
    dists(j,j)=inf;
end
global_timers(2)=global_timers(2)+toc(t1); t1=tic;
[~,ii]=sort(dists(:));
for j=1:length(ii)
    if (isinf(dists(ii(j))))
       global_timers(3)=global_timers(3)+toc(t1); t1=tic;
       return;
    end;
    [ak1,ak2]=ind2sub(size(dists),ii(j));
    k1=active_inds(ak1);
    k2=active_inds(ak2);
    if ((~returning_these_labels(k1))&&(~returning_these_labels(k2)))
        if ((weighted_counts(k1)>0)&&(weighted_counts(k2)>0)) % just to make sure (this was actually happening! probably should track down why)
            if (~was_already_attempted(attempted_comparisons,centers_active(:,ak1),centers_active(:,ak2),weighted_counts_active(ak1),weighted_counts_active(ak2),repeat_tolerance))               
                k1s(end+1)=k1;
                k2s(end+1)=k2;
                returning_these_labels(k1)=1;
                returning_these_labels(k2)=1;
            end;
        end;
    end;
end;
global_timers(5)=global_timers(5)+toc(t1); t1=tic;
...
if (size(X1,2)<size(X2,2))
    XX_sorted=sort(XX);
    cutoff=XX_sorted(2*size(X1,2));
    inds_for_comparison=find(XX<=cutoff);
else
    XX_sorted=sort(XX,'descend');
    cutoff=XX_sorted(2*size(X2,2));
    inds_for_comparison=find(XX>=cutoff);
end;

info.projection=XX;
info.projection_weights=WW;
info.inds_for_comparison=inds_for_comparison;

isocut4_opts.try_ranges=0;
[dip_score,cutpoint]=isocut4_matlab(XX(inds_for_comparison),WW(inds_for_comparison),isocut4_opts); %This is the core procedure -- split based on isotonic regression
...
[inds_thin,weights_thin]=thin5(samples,ceil(N/20));
samples_thin=samples(:,inds_thin);
true_labels_thin=true_labels(inds_thin);

figure;
ms_view_clusters(samples_thin(1:2,:),true_labels_thin);
title(sprintf('N = %d',length(true_labels_thin)));
drawnow;

opts.return_iterations=1;
opts.weights=weights_thin;
[labels_thin,info]=isosplit4(samples_thin,opts);
figure;
ms_view_clusters(samples_thin(1:2,:),labels_thin);
title(sprintf('isosplit4 -- N = %d',length(labels_thin)));
drawnow;
...
% tic;
% [labels,info]=isosplit2_cut4(samples);
% fprintf('isosplit2 cut4: num clusters = %d, num iterations = %d, elapsed = %g\n',max(labels),info.num_iterations,toc);
% figure;
...
tic;
opts.return_iterations=1;
[labels,info]=isosplit4(samples,opts);
disp(info.timers);
disp(global_timers);
fprintf('isosplit4: num clusters = %d, num iterations = %d, elapsed = %g\n',max(labels),info.num_iterations,toc);
figure;
ms_view_clusters(samples(1:2,:),labels); title(sprintf('isosplit4: N = %d',N));
drawnow;
...
[inds_thin,weights_thin]=thin5(samples,ceil(N/10));
samples_thin=samples(:,inds_thin);
...
opts.weights=weights_thin;
[labels_thin,info]=isosplit4(samples_thin,opts);
fprintf('thin isosplit4: num clusters = %d, num iterations = %d, elapsed = %g\n',max(labels),info.num_iterations,toc);
figure;
ms_view_clusters(samples_thin(1:2,:),labels_thin); title(sprintf('thin isosplit4: N=%d',length(samples_thin)));
drawnow;

%show_iterations(samples_thin,info);

tic;
[labels_thin,info]=isosplit4(samples_thin);
fprintf('thin isosplit4 - unweighted: num clusters = %d, num iterations = %d, elapsed = %g\n',max(labels),info.num_iterations,toc);
figure;
ms_view_clusters(samples_thin(1:2,:),labels_thin); title(sprintf('thin isosplit4 unweighted: N=%d',length(samples_thin)));
drawnow;
...
for j=1:length(info.iterations)
    ...
    subplot(1,2,1);
        ms_view_clusters(X(1:2,:),tmp.labels_before);
...
function labels=initialize_clustering(X,K)
[M,N]=size(X);
K=min(K,N);
inds=randsample(N,K);
centers=X(:,inds);
for it=1:1
    dists=zeros(length(inds),N);
    for m=1:M
        [xxx,yyy]=ndgrid(centers(m,:),X(m,:));
        dists=dists+(xxx-yyy).^2;
    end;
    dists=sqrt(dists);
    [~,min_inds]=min(dists,[],1);
    labels=min_inds;
    
    sums=zeros(M,K);
    counts=zeros(1,K);
    for m=1:M
        sums(m,:)=sums(m,:)+accumarray(labels',X(m,:),[K,1])';
        counts=counts+accumarray(labels',1,[K,1])';
    end
    centers(:,find(counts))=sums(:,find(counts))./repmat(counts(find(counts)),M,1);
end;
