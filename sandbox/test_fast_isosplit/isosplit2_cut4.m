function [labels,info]=isosplit2_cut4(X,opts)
...
XXs=sort(XX);
[dipscore,cutpoint]=isocut4_matlab(XXs); %This is the core procedure -- split based on isotonic regression
...
tA=tic;
[labels,info]=isosplit2_cut4(samples,opts);
fprintf('isosplit2: num clusters = %d, num iterations = %d, elapsed: %g\n',max(labels),info.num_iterations,toc(tA));
figure;
...
tA=tic;
[labels,info]=isosplit2_cut4(samples,opts);
fprintf('isosplit2_cut4: num clusters = %d, num iterations = %d, elapsed: %g\n',max(labels),info.num_iterations,toc(tA));
figure;
ms_view_clusters(samples,labels); title('isosplit2-cut4');
drawnow;
