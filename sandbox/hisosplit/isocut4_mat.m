function [dip_score,cutpoint,info]=isocut4_mat(samples,sample_weights,opts)
if nargin<1, test_isocut4_mat; return; end;
...
[~,N]=size(samples);
if (N==0)
    error('Error in isocut: N is zero.');
end;
num_bins_factor=1;
...
if (opts.try_ranges)
    ...
    for j=1:length(ranges_to_try)
        ...
        if (nargout>=3)
            [ds,cp,info2]=isocut4_mat(X,sample_weights,opts2);
        else
            [ds,cp]=isocut4_mat(X,sample_weights,opts2);
            info2=struct;
        end;
...
if (length(opts.range)==2)
    ...
    opts2=opts;
    opts2.range=[];
    [dip_score,cutpoint,info]=isocut4_mat(samples_in_range,sample_weights_in_range,opts2);
    if (nargout>=3)
...
tA=tic;
[dip_score,cutpoint,info]=isocut4_mat(X,sample_weights,opts);
%cutpoint=isocut(X,1);
%dip_score=cutpoint;
run_time=toc(tA);
