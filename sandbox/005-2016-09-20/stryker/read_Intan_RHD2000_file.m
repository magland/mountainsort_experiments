function output=read_Intan_RHD2000_file(filename)
...
if (data_present)
    ...
    % jfm took out the scaling so we can store raw as 16-bit unsigned!
...
output.notes=notes;
output.frequency_parameters=frequency_parameters;
%move_to_base_workspace(notes);
%move_to_base_workspace(frequency_parameters);

if (num_amplifier_channels > 0)
    output.amplifier_channels=amplifier_channels;
    %move_to_base_workspace(amplifier_channels);
    if (data_present)
        output.amplifier_data=amplifier_data;
        output.t_amplifier=t_amplifier;
        %move_to_base_workspace(amplifier_data);
        %move_to_base_workspace(t_amplifier);
    end
    output.spike_triggers=spike_triggers;
    %move_to_base_workspace(spike_triggers);
end
if (num_aux_input_channels > 0)
    output.aux_input_channels=aux_input_channels;
    %move_to_base_workspace(aux_input_channels);
    if (data_present)
        output.aux_input_data=aux_input_data;
        output.t_aux_input=t_aux_input;
        %move_to_base_workspace(aux_input_data);
        %move_to_base_workspace(t_aux_input);
    end
end
if (num_supply_voltage_channels > 0)
    output.supply_voltage_channels=supply_voltage_channels;
    %move_to_base_workspace(supply_voltage_channels);
    if (data_present)
        output.supply_voltage_data=supply_voltage_data;
        output.t_supply_voltage=t_supply_voltage;
        %move_to_base_workspace(supply_voltage_data);
        %move_to_base_workspace(t_supply_voltage);
    end
end
if (num_board_adc_channels > 0)
    output.board_adc_channels=board_adc_channels;
    %move_to_base_workspace(board_adc_channels);
    if (data_present)
        output.board_adc_data=board_adc_data;
        output.t_board_adc=t_board_adc;
        %move_to_base_workspace(board_adc_data);
        %move_to_base_workspace(t_board_adc);
    end
end
if (num_board_dig_in_channels > 0)
    output.board_dig_in_channels=board_dig_in_channels;
    %move_to_base_workspace(board_dig_in_channels);
    if (data_present)
        output.board_dig_in_data=board_dig_in_data;
        output.t_dig=t_dig;
...
if (num_board_dig_out_channels > 0)
    output.board_dig_out_channels=board_dig_out_channels;
    %move_to_base_workspace(board_dig_out_channels);
    if (data_present)
        output.board_dig_out_data=board_dig_out_data;
        output.t_dig=t_dig;
...
if (num_temp_sensor_channels > 0)
    if (data_present)
        output.temp_sensor_data=temp_sensor_data;
        output.t_temp_sensor=t_temp_sensor;
