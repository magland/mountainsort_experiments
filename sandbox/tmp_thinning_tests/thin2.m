...
%for aaa=0.05:0.05:0.95
    ...
    densities=max(1,counts);
    new_densities=min(25,densities);
    probs=min(1,new_densities./densities);
...
figure; plot(densities+randn(size(densities)),new_densities+randn(size(densities)),'r.');
