function [labels,info]=isosplit2b(X,weights,opts)
...
while 1
    ...
    if (opts.verbose)
        fprintf('Iteration %d: Comparing %d (%g) with %d (%g)...',info.num_iterations,k1,weighted_counts(k1),k2,weighted_counts(k2));
    end;
...
%XXs=sort(XX);
[ks_normalized,cutpoint]=isocutb(XX,WW); %This is the core procedure -- split based on isotonic regression
if (ks_normalized>opts.isocut_threshold)
...
[labels,info]=isosplit2b(samples,[],opts);
fprintf('isosplit2b: num clusters = %d, num iterations = %d\n',max(labels),info.num_iterations);
figure;
ms_view_clusters(samples,labels); title('isosplit2b');
drawnow;
